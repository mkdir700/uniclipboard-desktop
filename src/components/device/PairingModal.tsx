...
const PairingModal: React.FC<PairingModalProps> = ({ open, onClose, onConnectSuccess }) => {
  const [activeTab, setActiveTab] = useState<ConnectionTab>('manual')
  // 选中的网卡索引
  const [selectedInterfaceIndex, setSelectedInterfaceIndex] = useState<number>(-1)
  // IP 地址四段输入
  ...
  // 处理 IP 段键盘事件
  const handleIpSegmentKeyDown = (index: number, e: React.KeyboardEvent<HTMLInputElement>) => {
    // 按小数点或空格跳到下一个输入框
    if (e.key === '.' || e.key === ' ') {
      e.preventDefault()
      if (index < 3) {
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      const response: ManualConnectionResponse = await connectToDeviceManual(request)
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>选择一种方式来连接新设备</DialogDescription>
        </DialogHeader>
              ...
              {showErrorMessage && connectionState.status === 'failed' && (
                <div
                  className={`mb-4 overflow-hidden transition-all duration-300 ease-in-out ${
                    showErrorMessage ? 'max-h-24 opacity-100' : 'max-h-0 opacity-0'
                  }`}
                >
                ...
                <div className="space-y-4">
                  ...
                  {localInterfaces.length > 0 && (
                    <div>
                      ...
                      <Select
                        value={
                          selectedInterfaceIndex >= 0 ? String(selectedInterfaceIndex) : undefined
                        }
                        onValueChange={value => handleInterfaceSelect(parseInt(value, 10))}
                      >
                        <SelectTrigger>
                  ...
                  {/* IP 输入 */}
                  <div>
                    <label className="block text-sm font-medium mb-1.5">对向设备 IP 地址</label>
                    <div className="flex items-center gap-2">
                      {ipSegments.map((segment, index) => (
                        <React.Fragment key={index}>
                          ...
                          />
                          {index < 3 && <span className="text-muted-foreground text-lg">.</span>}
                        </React.Fragment>
                      ))}
                    </div>
