...
export const analyzeShortcutConflicts = (
  ...
  for (const s of resolved) {
    ...
    const layerMap =
      byKeyLayer.get(s.normalizedKey) ?? new Map<ShortcutLayer, ResolvedShortcut[]>();
  ...
  for (const [normalizedKey, layerMap] of byKeyLayer.entries()) {
    const layers = Array.from(layerMap.keys()).sort(
      (a, b) => LAYER_ORDER[b] - LAYER_ORDER[a]
    );
...
export const getCandidateKeyIssues = (
  ...
  const candidateLayer =
    resolved.find((s) => s.id === candidate.id)?.layer ??
    DEFAULT_SCOPE_LAYER[candidate.scope];

  const sameScope = resolved.filter(
    (s) =>
      s.id !== candidate.id &&
      s.scope === candidate.scope &&
