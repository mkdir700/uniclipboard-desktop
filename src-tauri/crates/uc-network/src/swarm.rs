...
use uc_core::network::{
    ClipboardMessage, ConnectedPeer, DiscoveredPeer, NetworkEvent, NetworkStatus, PairingMessage,
    ProtocolMessage,
};
...
impl NetworkManager {
        ...
        loop {
            ...
            match receiver.handle_frame(frame).map_err(|e| e.to_string())? {
                ...
                FrameHandleResult::TransferComplete => {
                    ...
                    let clipboard_message = uc_core::network::ClipboardMessage {
    ...
    pub async fn run(&mut self) {
        ...
        // TODO: This should be passed as a parameter to NetworkManager
        let local_ip = "0.0.0.0".to_string(); // Fallback to 0.0.0.0
    ...
    async fn handle_swarm_event(
        ...
        match event {
                                    ...
                                    match protocol_msg {
                                        ...
                                        ProtocolMessage::Pairing(PairingMessage::Confirm(
                                            ...
                                            if confirm.success {
                                                ...
                                                let ack = uc_core::network::PairingConfirm {
            ...
            SwarmEvent::Behaviour(super::behaviour::UniClipboardBehaviourEvent::Identify(
                ...
                // TODO: This should be handled by the application layer, not uc-network
                if let (Some(device_id), Some(device_name)) = (device_id, device_name) {
                    // Emit event for application layer to handle database update
                    let _ = self
                        .event_tx
                        ...
                        if let Some(addr_str) = discovered.addresses.first() {
                            if let Ok(addr) = addr_str.parse::<Multiaddr>() {
                                ...
                                if !self.dialing_peers.contains(&peer_id) {
                                    ...
                                    let attempt =
                                        self.reconnect_attempts.entry(peer_id).or_insert(0);
                                    ...
                                    *attempt += 1;
                                    ...
                                    if backoff_delay.is_zero() {
                                        if let Err(e) = self.swarm.dial(addr) {
                                            warn!(
                                                "Failed to dial {} for reconnection: {}",
                                                peer_id, e
                                            );
                                            self.dialing_peers.remove(&peer_id);
                                        } else {
                            ...
                            debug!(
                                "No cached addresses for {}, waiting for mDNS rediscovery",
                                peer_id
                            );
                        }
                    } else {
    ...
    async fn handle_command(&mut self, command: NetworkCommand) {
        match command {
            ...
            NetworkCommand::SendPairingChallenge {
                ...
                match peer_id.parse::<PeerId>() {
                    Ok(peer) => {
                        let challenge = uc_core::network::PairingChallenge {
            ...
            NetworkCommand::SendPairingResponse {
                ...
                match peer_id.parse::<PeerId>() {
                    Ok(peer) => {
                        let response = uc_core::network::PairingResponse {
                            session_id: session_id.clone(),
                            pin_hash,
                            accepted,
                        };
            ...
            NetworkCommand::RejectPairing {
                ...
                match peer_id.parse::<PeerId>() {
                    Ok(peer) => {
                        let confirm = uc_core::network::PairingConfirm {
                            session_id: session_id.clone(),
                            success: false,
