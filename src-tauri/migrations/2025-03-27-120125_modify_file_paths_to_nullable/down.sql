...
-- 创建新表，字段 local_file_path 和 remote_record_id 为NOT NULL的TEXT类型
...
-- 复制数据到新表，确保NULL值转换为空字符串
INSERT INTO clipboard_records_new 
SELECT id, device_id, IFNULL(local_file_path, ''), IFNULL(remote_record_id, ''), content_type, content_hash, is_favorited, created_at, updated_at, active_time
