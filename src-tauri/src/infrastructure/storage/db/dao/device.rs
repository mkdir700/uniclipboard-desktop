use crate::infrastructure::storage::db::models::device::{DbDevice, NewDevice, UpdateDevice};
...
pub fn get_self_device(conn: &mut SqliteConnection) -> Result<Option<DbDevice>> {
    devices::table
        .filter(devices::self_device.eq(true))
        .first(conn)
        .optional()
        .context("Failed to get self device")
}
